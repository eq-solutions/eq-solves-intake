...
- **EQ Capture** (future) — a standalone OCR surface for inputs that don't go through Cards. Forwarded supplier emails, paper handed across a desk, PDFs the bookkeeper drags in. The OCR engine already runs inside Cards (mobile ML Kit + Claude Vision via a Supabase Edge Function for web); Capture is the work of generalising it to arbitrary inputs and arbitrary canonical entities. Easy to light up when there's a reason. Not priority.
