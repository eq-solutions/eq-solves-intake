...
**Lesson captured in memory:** `[[feedback_use_plain_language]]` (jargon habit Royce flagged in the same session) + `[[project_eq_service_canonical_migration]]` (don't build parallel strict importers; fold canonical-shape persistence into the existing surface).
