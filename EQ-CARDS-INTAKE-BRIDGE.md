...
- **2026-08-02 (correction — two claims above are stale, checked against live git/DB/deploy, not docs):**
  - **The Cards→Field SSO sending half described as unbuilt elsewhere in this doc's era is live.** `token-exchange.ts` in eq-shell resolves `worker_id` from `public.workers` and mints it into the `aud='field'` JWT (PR [#812](https://github.com/eq-solutions/eq-shell/pull/812), "one-login P1", 2026-07-13), with earned-access gating via `field_access_unlocked_at` added by PR [#1145](https://github.com/eq-solutions/eq-shell/pull/1145) (2026-08-01) — that's the current live Netlify deploy on core.eq.solutions. eq-field's `verify-pin.js`/`auth.js`/`people.js` consume `app_metadata.worker_id` as `canonical_user_id` and link the Field person to the canonical worker registry, confirmed on `main` there too.
  - **Reshape-out is not narrow.** `eq-format-ui/src/derive/registry.ts` (eq-platform workspace) registers **12** live profiles — SimPRO, Xero, MYOB, and four Equinix-specific formats — not a handful of SimPRO quote variants.
  - **Still genuinely open, found the same pass:** of 97 `public.workers` rows on jvkn, only 53 (55%) have a linked `shell_control.users` row — the other 44 have a real tenant `staff_id` (none older than 90 days) but no Shell login to mint any JWT from in the first place. Per Royce (2026-08-02): expected, team is being onboarded in tranches, not a defect — worth a check-in only if unlinked-worker growth outpaces tranche throughput. The worker-pool-vs-control-plane architecture question this doc was written to answer is otherwise unresolved by this correction — that needs its own re-audit, not assumed current from either version of this doc.
