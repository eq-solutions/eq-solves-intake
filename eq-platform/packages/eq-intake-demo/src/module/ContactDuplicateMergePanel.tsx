...
function MergePanel({
  ...
  mergeBusy: boolean;
  mergeErr?: string;
  merged?: { survivor_contact_id: string; movedTotal: number };
  onPreview: () => void;
...
function ContactAdvisoryPanel({
  ...
  merged: Record<string, { survivor_contact_id: string; movedTotal: number }>;
...
export function ContactDuplicateMergePanel({ supabase, canMergeContacts }: ContactDuplicateMergePanelProps): JSX.Element | null {
  ...
  const [merged,            setMerged]            = useState<Record<string, { survivor_contact_id: string; movedTotal: number }>>({});
