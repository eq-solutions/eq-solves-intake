-- ============================================================================
-- 026 — Fix eq_intake_commit_batch_core upsert conflict target
-- ============================================================================
-- Applies to: sks-canonical (ehowgjardagevnrluult) and eq-canonical.
--
-- Problem: migration 008 wrote the 'upsert' CASE in eq_intake_commit_batch_core
-- using ON CONFLICT (customer_id / contact_id / site_id) — the UUID PK.
-- When rows arrive from intake, they have a freshly generated UUID (no existing
-- PK value), so the PK conflict never fires and every re-import inserts new rows.
--
-- Fix: change the conflict target to the partial unique indexes already
-- created by migration 022:
--   customers_tenant_external_id_uidx  ON (tenant_id, external_id) WHERE external_id IS NOT NULL
--   sites_tenant_external_id_uidx      ON (tenant_id, external_id) WHERE external_id IS NOT NULL
--   contacts_tenant_external_id_uidx   ON (tenant_id, external_id) WHERE external_id IS NOT NULL
--
-- Rows with external_id IS NULL still INSERT (no match possible — correct).
-- Rows with a matching (tenant_id, external_id) UPDATE with COALESCE merge
-- (incoming non-null wins; existing non-null preserved if incoming is null).
--
-- This makes all intake runs idempotent: re-importing next week's SimPRO
...
BEGIN
  ...
  IF v_import_mode = 'replace' THEN
    ...
    END IF;
    EXECUTE format(
      'DELETE FROM app_data.%I WHERE tenant_id = $1 AND imported_from = $2',
      p_table
    ...
    CASE p_table
      ...
      WHEN 'customers' THEN
        IF v_import_mode = 'upsert' THEN
          ...
          -- Conflict on (tenant_id, external_id) WHERE external_id IS NOT NULL.
          -- Rows without an external_id still INSERT (no match possible).
